...
| Viewed-history recency | Recent prioritizes the enabled Recent Files plugin's file-open sequence, appends native history, and uses modified-time roots only as a no-history fallback | Native and optional-plugin history fixtures exercise priority, configuration, deduplication, and fallback behavior |
