...
## Dismiss and restore in 0.3.13

For an intentionally deleted file, choose **Dismiss from review** beside **Locate file** on its unresolved row. This immediately hides the selected processing entries from the ordinary File review list, search results and day counts. It does not delete, move or edit any file, or alter a processing dashboard. Other missing entries remain visible; dismissal is always explicit and reversible, with no confirmation modal.

Use **Show dismissed (count)** in the review controls to see dismissed entries separately, with their retained processing history. Search, workflow filters and processing-day sections also work in that view. **Restore to review** returns an entry to active review and resumes ordinary location checks; use **Back to active review** to return to the normal list. If the file is still missing, Restore shows Locate and Dismiss again. If a unique verified match is now available, normal recovery may reconnect it after Restore.

Dismissal applies to every workflow history in the selected row, keyed by its processing event and exact recorded output revision. A genuinely new processing event remains visible, even at the same file path; its older dismissed events remain in the dismissed view. Reusing an event ID with a different dashboard output path does not inherit dismissal, including a producer rewriting its output path to a prior corrected location. Repeated refresh, file reappearance, plugin reload and ordinary journal synchronization do not silently restore dismissed history.

Both views remain limited to current output entries in the same nine dashboards. If a source row disappears or its dashboard becomes unreadable, it appears in neither view; its saved dismissal remains in the journal and applies when that exact source entry returns. Restore is available when the entry is present in a readable dashboard. This keeps journal-only records from creating membership.

Visibility state and its update time persist in the existing review journal, independently of location/fingerprint update times. A newer fingerprint update cannot undo a dismissal or Restore received from another device. All devices should run 0.3.13 or newer to preserve these fields. Normal Sync conflicts and device clock ordering still apply; physical second-device acceptance remains untested. Dismissed entries skip live file binding, fingerprint reads and automatic recovery scans until explicitly restored. Unreadable saved history or failed persistence reports an error; failed visibility writes are rolled back in memory.

Focused automated checks cover dismissal and Restore across refresh/reload, combined workflow histories, unchanged source documents, current-table membership, removed/restored sources, reused IDs and new events, independent synchronized visibility updates, write failures, stale/recovered rows, zero recovery work for dismissed entries, and active versus dismissed day counts.

Live validation used isolated disposable records in the installed plugin: two active rows became one active plus one dismissed row containing two workflow histories; the dismissed view rendered its Restore control and pressed toggle state; Restore survived a fresh store instance and returned both rows and original day counts. Source fixture content was unchanged by dismissal/Restore. The Dismiss layout was visually inspected; native click automation then reported unavailable/stale window controls, so the live state transitions were exercised through the installed store methods rather than claimed as a complete physical click cycle. Fixtures were removed through Obsidian Trash. The real review remains at 477 active and zero dismissed files. All 205 tests, production build and dependency audit pass.
